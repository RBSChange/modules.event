<?php
/**
 * event_ListBlocktemplatesService
 * @package modules.event.lib.services
 */
class event_ListBlocktemplatesService extends BaseService implements list_ListItemsService
{
	/**
	 * @var event_ListBlocktemplatesService
	 */
	...
	/**
	 * @return event_ListBlocktemplatesService
	 */
	...
	public final function getItems()
	{
		...
		if (!isset($this->itemArray[$templatePrefix]))
		{
			...
			$paths = FileResolver::getInstance()->setPackageName('modules_event')->getPaths('templates');
			...
			foreach ($paths as $path)
			{
				...
				while (false !== ($entry = $dir->read()))
				{
						...
						&& f_util_StringUtils::endsWith($entry, '.all.all.html', f_util_StringUtils::CASE_SENSITIVE))
					{
						...
						$label = $ls->transBO('m.event.list.blocktemplates-' . strtolower($value), array('ucf'));
